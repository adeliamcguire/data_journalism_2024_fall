...
**Answer** I am seeing numerous problems with the data and the way in which it is formatted and organized. The first issue that sticks out to me is that there is no header row present which further confuses how we interpret the data in front of us. Another problem that must be addressed is the rows with NA's as well as the office names mixed up with candidate names. To begin altering this data so that it easier to read and cleaner in how its presented we should start by removing unnecessary rows. 
...
**Task** Get rid of rows we don't need by running the codeblock below. What problems remain? 

**Answer** After making alterations to the code, it is clear we have improved the organization and formatting of the data by incorporating numbers and candidates. That being said, better headers (something that's not X2 - X34) are still needed. Additionally, in order to overcome the problem of extracting the party value we need to discover a way to split up the candidate column. If we were to split the party values out and incorporate headers with precinct and vote type data. Although we have made progress by adding numbers and candidates, we still need to make a party column, change the wording of the headers, and add an office column. 
...
**Task** Run the codeblock below. What changed, and what issues or problems remain? 

**Answer** Overall, the changes we made in the former codeblock have immensely improved the readability and formatting of this data. We now have a office column, candidate column, and party column. The office column was an important change because we could see what candidates were "U.S. Senate" and which were "U.S. House"  The columns headers that were formerly labeled "X2" to "X7" were changed to be "better" names, however, if you go to the second and third page of this data you will see that columns "X8" to "X34" have the same "weak" or "bad" header names. Additionally, some entire columns such as, "X29", "X31","X32","X33", and "X34, are entirely made up of NA's within their column. Beyond doing manual cleaning in R Studio, in order to fully clean up this data we would need to make a single row for each precinct.
