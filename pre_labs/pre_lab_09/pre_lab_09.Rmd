...
To do that, click on the "PNG" icon. Finally, click the "Download image" button. Copy the url of the graphic you published here. It will begin with <https://datawrapper.dwcdn.net/>. Don't copy the browser's current url.

**Answer** My completed, published graphic can be accessed at <https://www.datawrapper.de/_/oHk6D/>
...
```{r}
md_gov_county_cleaned <- md_gov_county |>
   select(-total_votes_2, -moore_percentage_2)
```

```{r}
summary_results <- md_gov_county_cleaned |>
  summarise(total_votes = sum(cox + moore + lashar + wallace + harding + `write_ins`),
            moore_percentage = (sum(moore) / total_votes) * 100)

print(summary_results)
write_csv(md_gov_county_cleaned, "data/md_gov_county.csv")
```
...
**Answer** Describe the changes from the previous map: Prior to hitting the "crop to data" button the map showed the entire United States as a grey color representing it had no data with the only colored area, Maryland, being very small and hard to identify. Once I selected "crop to data" the entire format and appearance of the map was altered. The map zoomed in on the state of Maryland, clearly showing the the different counties and the colors associated with them. This feature of datawrapper works to change the map so that it is focused specifically on the region(s) that pertain to your specific data. 
...
The impacts of the changes of the finalized map are night and day from the map we started with. The general visual appearance improved significantly as the map that was once grey and showed the entirety of the U.S. with Maryland barely visible, is now detailed, focused and full of color. After taking the step to enable the "Hide regions without data" feature, the map zoomed in on Maryland which helped give us that clear representation of the counties and each of their respective vote percentages.The color legend was also altered to better indicate the full extend of Moore's support across Maryland's counties. Tooltips were also enhanced so that the hover feature was beneficial and educational rather than annoying and unnecessary. Overall, in each of these rather small changes, the map became user-friendly and even was able to tell a story in itself. 
...
My completed and published map can be accessed through this link: <https://www.datawrapper.de/_/RnpJT/>. 
