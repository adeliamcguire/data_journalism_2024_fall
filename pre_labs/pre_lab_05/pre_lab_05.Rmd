...
**Task** Load the five CSV files. Do they have the same column structure?

**Answer** Yes, all of the columns (YEAR, COUNTY, DEM, REP, LIB, OTH, UNA, TOTAL) are the same in their structure. Each of the eight columns within the five data sets are all united in their header formatting/presentation. 
...
**Answer** The function bind_rows() takes multiple data frames and combines them into one data frame by vertically stacking them. As we learned in class, this function requires that all the input data frames have the same column name in order to be properly combined. The list() function works to take the data frames that are going to be combined and make them into a list, allowing for us to combine more than two data frames at once. Ultimately, this data frame will have all rows from each of the individual data frames which helps us by making it easier to analyze combined voter registration data across multiple years. 
...
**Answer** The population data for Maryland 2020 counties was successfully loaded. It is made up of 24 rows and two columns, one of which is labeled POP2020 and the other of which is labeled COUNTY. 
...
**Answer** After running the code below, I can see that their are 24 rows remaining. Considering that 24 is the number of rows we had with a left join with population on the left, 24 is both the correct number of rows and the number we expected to have. 
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? 

**Answer** When we create a new dataframe that has population and voter data joined together, we receive a message that reads as follows, "Joining with `by = join_by(COUNTY)`" This message tells us that the join operation was performed using the COUNTY column as the key, in other words, the data from both dataframes was matched based on the values in the COUNTY column. It also confirms that the join was successful by highlighting which specifc column was used for the connection.
...
**Answer** Through using the mutate function, I have determined that Prince George's County has the highest 18+ Black population percentage at 60.9035922%. The same remains true for the Maryland county with the highest percentage of democrats, as Prince George's County sits in first place with a total of 78.29988% of their population identifying as members of the democratic party. 
...
**Answer** The Maryland county with the highest 18+ Asian population is Howard County with a total percentage of 19.3671955. The data also tells me that 52.37740% of	the Howard County population are Democrats. While comparing these results with the results from the previous question, it is clear that while Prince George's County had significantly higher percentages for both Black population and Democratic affiliation, Howard County contains a high Asian population percentage (in terms of Maryland counties) but a far lower percentage of Democrats relative to Prince George's County. This difference highlights how different racial demographics can correlate with political affiliations in Maryland. For example, the data on Prince George's County shows both high racial representation and strong Democratic support, whereas Howard County shows a very different demographic composition with a relatively lower percentage of Democrats despite having the highest Asian population percentage in the state.
...
**Answer** The datatype of the ZCTA5N column is <dbl> or a numerical datatype. 
...
**Answer** Now, after altering the code to change the datatype of ZCTA5N, ZCTA5N is a <chr> or character datatype rather than a <dbl> or numerical datatype. Changing the datatype of certain columns/pieces of data can be really important in allowing us to properly join dataframes together. 
