...
```{r echo=FALSE}
#census_api_key("3736f509811880a48c26c37aadd7dfee35905e3b", install=TRUE)
```
...
**Answer** Nevada is the state with the largest percentage growth with a total percent change of 35.1453292%. Nevada's population went from 1998257 in 2000 to 2700551 in 2010. 
...
**Answer** The Maryland County with the highest median household income is Howard County, Maryland. Howard County has a total estimated median income of $140,971. 
...
**Answer** Through observing the Household Income by County in Maryland chart based on the 2018-2022 American Community Survey, it is evident that difference between Montgomery County and Calvert County are not very significant. The margin of error bars for Montgomery County and Calvert County  overlap which means the difference between the two counties is within the margin of error, revealing that the differences are not statistically significant.

Is the difference between Howard and everyone else significant? 

**Answer** The difference in median annual income between Howard and the other Maryland counties on the Household Income by County in Maryland chart is fairly significant. While many of the other counties margin or errors overlap and are positioned similarly, Howard's margin of error is an outlier in where it sits. That being said, despite standing out as the county with the largest income, the size of Howard's margin of error seems to be a fairly typical range similar to many of the other counties.
...
**Task** Run the following codeblock to join the data, rename the population estimates and then calculate the values we need. Notice how we select *after* we do the initial calculations so we only see the columns we're interested in (and drop the ones we don't want to see). Also note how the select function has the minus sign (-) in front of some columns? That means we don't want to see them in our result. 
Did any counties see their median incomes fall from 2010-2022? Is the change outside the margin of error for both? 

**Answer** Even the county that had the smallest percentage change, Caroline County, has a median income change of a 11.10053% increase. Not a single Maryland county saw their median income fall from 2010 to 2022. The data tells me that every single county has recovered from the financial crisis in the late 2000s. 
...
**Answer** Of the top 20 zip codes with the largest amounts, ZCTA5 21202 is the only zip code with an estimated median household income less than 50k with an estimated median household income of $47,848. Upon further research I found that the zip code 21202 is associated with Baltimore, Maryland.
...
**Task** Arrange md_zips_with_acs by estimate showing the highest median household income first. What questions do you have about these top zip codes? 

**Answer** ...

```{r}
md_zips_sorted <- md_zips_with_acs |> 
  arrange(desc(estimate))
```

Through using the arrange function I was able to create another dataframe that includes the estimates from md_zips_with_acs sorted from highest median household income to lowest median household income. One question that immediately came to mind when looking over this data was how many of the zip code were associated with areas/territories within Howard County. Considering that Howard County is the Maryland county with the highest median household income I would have to assume that there are quite a few Howard County based zip codes in the top zips with the highest median household income. Some additional questions I would want to find out about these zip codes is what the demographics of residents in these top zip codes are and what geographic or environmental related factors could potentially contribute to higher median household incomes in certain zip codes compared to others. 
