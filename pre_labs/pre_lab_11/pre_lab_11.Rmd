...
** Double click on page and navigate to "view source" tab... **
...
**Answer** Through running the following codeblock I am able to extract all the tables in the HTML and save it as a functioning dataframe. Based on my initial observations of the 7 columns incorporated within this dataframe I am identifying a multitude of issues with the general organization and formatting of the columns. Though we now have a proper dataframe, the first issue that needs to be addressed is in regard to the column names and how they include lengthy headers consisting of both the candidate name and their designated party in the same cell. Another error I am seeing lies within the 25th row in how it has both the total number and total percent in the same cell. In terms of what I notice within the the code itself, the use of results[[1]] stands out to me in how it is used to select the first table from the list of tables returned by html_table().These double brackets [[ ]] are used to extract elements from a list ( 1 refers to the first element of that list). While their are issues present in this dataframe that resulted in the process extracting tables from HTML, they are easily reversible with basic cleaning and restructuring. 
...
**Task** Run the following code to extract the table we want, clean up the names and remove the total row. Look at the rename() function - what do you think the numbers 2 through 6 are referring to? 

**Answer** The following code successfully extracts and cleans up our desired table. As I mentioned in my answer to the former question, the column names were initially flawed in their inclusion of unnecessary details liek full candidate names and their political party affiliation. We fixed this by using the rename() function to simplify the columns names to that of the last name of the candidate running, all in all making the dataframe far more cleaner and easy to decipher. For example we took "Donald J. Trump and JD Vance Republican" and cleaned it up to be "trump". In order to address the confusing, cluttered last row (25th row) that included both the total and percent in the same cell, we eliminated the 25th row all together using slice(-25). We also mutated our columns to make them numeric through using to gsub to find all of the commas from the vote counts and replace them with a space. This ensured that our data was uniform and properly formatted for analysis. The numbers 2 through 6 in the rename() function refer to the designated column positions in the original dataframe, which are being mapped to simpler, more understandable and relevant names. With each of these changes, we worked to fine tune the dataframe so that it is now in a much more usable format, and we have a cleaned dataframe containing information on the Maryland presidential election results. 
...
results <- results[[1]] |>
  clean_names() |>
  rename(harris = 2, trump = 3, oliver = 4, stein = 5, kennedy = 6, sharon_harris = 7) |>
  slice(-25) |>
  ...
  mutate(sharon_harris = as.numeric(gsub(",","", sharon_harris)))
...
**Answer** The output suggests that there are multiple <ul> tags in the HTML. Upon running this codeblock, 55 <li> elements are returned, each of which can be assumed to be contained within several un-ordered lists.  Each <li> tag corresponds to a list item within an un-ordered list (<ul>), in other words, each <li> tag we extracted is representative of a list item which indicates that there are several <ul> structures in the HTML that contain their own set of list items. In order to determine the exact number of <ul> tags in the HTML source we would need to take steps to create additional code ( (html_elements('ul') )to reveal this however, as I said before, the output makes it very clear that there are a multitude of <ul> elements based on the presence of 55 <li> elements.
...
**Answer** The output from the following code shows the raw text from each <li> tag in the HTML. This includes a mix of menu items, committee names, and other site links. While the first few entries we see are related to navigation links, later items include a list of committees. There is also extraneous text at the end that indicates that the list includes various types of links, not just committees. To alter our data so that it focuses solely on the committee names, further cleaning of the data would be required.The issue that is presenting itself here is that these committee names are mized with irrelevant and undesired links.

In order to address this we will use the tibble() function to structure the data into a dataframe. Another noteworthy change we will be seeing is the application of html_attr() with the 'href' attribute, giving us the ability to extract the URLs associated with the <a> tags and allowing us to isolate the committee links. On top of cleaning up the data by only keeping the relevant committee names and their links, we are also converting them into a more user friendly format. 
...
### Task 11: Turn the contents of the <li> and <a> tags into a dataframe **Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output:

**Answer** As we learned in class today, the order of opperation here matters. As you can see in the output, we are looking for li tags that have an a tag inside of it, meaning we are specifically targeting <li> tags that contain <a> tags within them. Additionally,  we are using a function called "tibble" which works to make a dataframe for us and convert the extracted data into a clean and tidy dataframe format with two columns. One of these columns is for the committee names and the other one is for their associated URLs. A series of functions come into play in this codeblock to get us the output we are seeing now. We get the name of the committees using html_text.The html_text(trim = TRUE) function retrieves the committee names, removing extra unnecessary and potentially confusing whitespace and blank lines. The URL's from the href attribute of the <a> tags are extracted through using html_attr("href"). The output of this dataframe includes all the committee names and their links, but its important ot note that not all of these URL's are necessary particularity relevant. The data that we don't want can be differentiated from the data that we do want because it has a URL that begins with a back splash (/) which is a symbol indicating internal links. We address this issue in Task 12 and are able to filter out all of the committees with URL's that begin with a backsplash through using "filter(!str_detect(url, "^/"))". 
